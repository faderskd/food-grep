...
class RestaurantsTest(BaseIntegrationTest):
    ...
    def test_should_edit_restaurant(self):
        ...
        self.assertEqual(self.test_client.get('/api/restaurants').get_json(),
                         [
                             {'name': 'test_restaurant', 'url': 'http://folkgospoda.pl',
                              'requirements': {'lunchRegex': '.*lunch today.*', 'imageUrlRegex': '',
                                               'time': '0 9 * * *'}}
                         ])
    ...
    def test_should_throw_error_during_editing_when_restaurant_does_not_exist(self):
        # when
    ...
    def test_should_delete_restaurant(self):
        ...
        # when
        response = self.test_client.delete('/api/restaurants/' + self.restaurant_data['name'])

        # then
        self.assertEqual(response.status_code, 200)

        # and
        restaurants = self.test_client.get('/api/restaurants').get_json()
        self.assertEqual(len(restaurants), 0)

    def test_should_throw_error_during_deleting_when_restaurant_does_not_exist(self):
        # when
        response = self.test_client.delete('/api/restaurants/' + self.restaurant_data['name'])
